...
public sealed class FeedTypeManager : IFeedTypeManager
{
    ...
    public async Task<FeedType> AddAsync(FeedType requestedFeedType, CancellationToken cancellationToken)
    {
        ...
        FeedType entity = new()
        {
            Description = requestedFeedType.Description
        };
        ...
        EntityEntry<FeedType> changes = await _dbContext.AddAsync(entity, cancellationToken).ConfigureAwait(false);
    ...
    public async Task<FeedType> UpdateAsync(FeedType requestedValues, CancellationToken cancellationToken)
    {
        _logger.LogInformation("Updating the feed type with ID {Id}...", requestedValues.Id);

        FeedType? entity = _dbContext.FeedTypes
            .FirstOrDefault(feedType => feedType.Id == requestedValues.Id);
        if (entity == null)
        {
            _logger.LogWarning("An attempt was made to update a non-existing feed type {@Request}", requestedValues);
            throw new EntityNotFoundException<FeedType>(requestedValues.Id);
        }
